...
impl Tool for TaskTool {
    ...
    /// Humanize: subagent + prompt + first 30 lines of result
    fn humanize(&self, args: &Value, result: &ToolResult) -> Option<String> {
        let subagent = args.get("subagent_type").and_then(|v| v.as_str()).unwrap_or("?");
        let desc = args.get("description").and_then(|v| v.as_str()).unwrap_or("");

        if result.is_error {
            return Some(format!("task[{}] \"{}\" → err: {}", subagent, desc, result.output));
        }
