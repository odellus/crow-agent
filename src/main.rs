...
use crow_agent::{
    ...
    run_stdio_server, tools, Telemetry,
};
...
static INTERACTION_GUARD: std::sync::OnceLock<
    std::sync::Mutex<Option<Arc<tokio::sync::Mutex<crow_agent::InteractionGuard>>>>,
...
impl CrowCli {
    async fn new(
        ...
        let todo_store = tools::TodoStore::new();
        ...
        let (agent, coagent_tool_registry, coagent_tools) = if agent_config.has_coagent() {
            ...
            // The agent_name column distinguishes primary vs coagent traces
            let coagent_session_id = session_id.clone();
            ...
            let coagent_registry = tools::create_coagent_registry(
                working_dir.clone(),
                coagent_session_id,
                todo_store.clone(),
                false,
        ...
        let tool_registry = tools::create_full_registry_async(
            working_dir,
            session_id,
            todo_store,
            agent_registry,
    ...
    async fn run_agent(
        ...
        let guard = Arc::new(Mutex::new(InteractionGuard::new(
            telemetry.clone(),
            Some(model),
        )));
            ...
            while let Some(event) = rx.recv().await {
                match &event {
                    ...
                    AgentEvent::CoagentStart { primary, coagent } => {
                        println!(
                            "\x1b[35m⟳ Coagent {} reviewing {}...\x1b[0m",
                            coagent, primary
                        );
                        guard_clone
                            .lock()
                            .await
                            .push_coagent_start(coagent, primary);
                    }
    ...
    fn load_session_messages(
            ...
            match role {
                ...
                "assistant" => {
                    ...
                    if let Some(tool_calls_json) = tool_calls {
                        ...
                        if let Ok(calls) = serde_json::from_value::<
        ...
        if last_response.is_some() || last_tool_calls.is_some() {
            ...
            if let Some(ref tool_calls_json) = last_tool_calls {
                if let Ok(calls) = serde_json::from_str::<
                    Vec<async_openai::types::ChatCompletionMessageToolCall>,
                >(tool_calls_json)
                {
                    builder.tool_calls(calls);
    ...
    async fn chat_with_history(&mut self, message: &str, session_id: Option<&str>) -> Result<()> {
        ...
        if let Some(sid) = session_id {
            if let Some(loaded_thread) = self.telemetry.load_thread(sid)? {
                println!(
                    "Continuing session: {}",
                    &loaded_thread.id[..8.min(loaded_thread.id.len())]
                );
                println!(
                    "Loaded {} messages from previous session",
                    loaded_thread.messages.len()
                );
    ...
    fn print_result(&self, result: &RunResult) {
        match result {
            RunResult::Complete {
                ...
                println!(
                    "\x1b[36m✓ Task complete ({} turns): {}\x1b[0m",
                    total_turns, summary
                );
            }
...
fn build_provider(cli: &Cli) -> Result<Arc<ProviderClient>> {
    ...
    if let Some(ref base_url) = cli.base_url {
        let config =
            ProviderConfig::custom(&cli.provider, base_url, "UNUSED_API_KEY_ENV", &cli.model);
        return Ok(Arc::new(
...
fn show_traces(
    ...
    let collected: Vec<TraceRow> = stmt
        .query_map([], |row| {
            Ok((
                row.get(0)?,
                row.get(1)?,
    ...
    if let Ok(messages) = serde_json::from_str::<Vec<serde_json::Value>>(&request_messages) {
        for msg in &messages {
            ...
            let display = if full || content.len() <= 500 {
                ...
                format!(
                    "{}...\n({} chars total, use --full to see all)",
                    &content[..500],
                    content.len()
                )
            };
...
async fn main() -> Result<()> {
    ...
    if cli.list_agents {
        ...
        for agent in agents {
            ...
            println!("  {:12} [{:8}]{} - {}", agent.name, mode, builtin, desc);
        }
        return Ok(());
    }
